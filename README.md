...
1. Download bundle for TMC local from [customerconnect.vmware.com](https://customerconnect.vmware.com).  Current Bundle is 1.0.1
2. Download tanzu CLI from [customerconnect.vmware.com](https://customerconnect.vmware.com) Look under VMware Tanzu Kubernetes Grid > 2.x > Product Binaries.  Can use 2.1, 2.2 versions of Tanzu CLI.  For 2.3 and above the CLI is now under its own category.

## Install Tanzu CLI

1. Install Tanzu CLI Example (refer to official documenation for more information)
```
mkdir tanzu
tar -zxvf tanzu-cli-bundle-linux-amd64.tar.gz -c tanzu/
cd tanzu/cli
sudo install core/v0.29.0/tanzu-core-linux-amd64 /usr/local/bin/tanzu
tanzu init
tanzu plugin clean
tanzu plugin sync
tanzu plugin list 
...
```
mkdir tmcsm
tar xvf bundle-1.0.1.tar -c tmcsm/
cd tmcsm
...
tanzu package install cert-manager --package cert-manager.tanzu.vmware.com --namespace tkg-packages --version 1.7.2+vmware.3-tkg.2
...
1. Version 1.0.1 needs a local cluster issuer to sign certificates used by TMC Local
2. Create a self signed CA for TMC Cluster Issuer
```
...
```
4. Create Extensions file for Certificate
```
cat > v3.ext <<-EOF
...
2. Create tmc-local namespace
```
kubectl create ns tmc-local
```
3. Add TMC SM Package Repository to tmc-local namespace
```
tanzu package repository add tanzu-mission-control-packages --url "harbor.vdoubleb.com/tmcsm/package-repository:1.0.1" --namespace tmc-local
...
1. You can view all the possible Key Values on the [Offical Documentation](https://docs.vmware.com/en/VMware-Tanzu-Mission-Control/1.0/tanzumc-sm-install/install-config-keys.html)
2. You can also use the tanzu package available get command
```
tanzu package available get tmc.tanzu.vmware.com/1.0.1 -n tmc-local --values-schema
```
...
```
            | 3:38:52AM: ---- waiting complete [26/26 done] ----
            | Succeeded
...
tanzu package installed list -n tmc-local
  ...
  contour                       contour.bitnami.com                                12.2.6           Reconcile succeeded
  kafka                         kafka.bitnami.com                                  23.0.7           Reconcile succeeded
  kafka-topic-controller        kafka-topic-controller.tmc.tanzu.vmware.com        0.0.22           Reconcile succeeded
  minio                         minio.bitnami.com                                  12.6.12          Reconcile succeeded
  pinniped                      pinniped.bitnami.com                               1.2.8            Reconcile succeeded
  postgres                      tmc-local-postgres.tmc.tanzu.vmware.com            0.0.67           Reconcile succeeded
  postgres-endpoint-controller  postgres-endpoint-controller.tmc.tanzu.vmware.com  0.1.47           Reconcile succeeded
  s3-access-operator            s3-access-operator.tmc.tanzu.vmware.com            0.1.24           Reconcile succeeded
  tanzu-mission-control         tmc.tanzu.vmware.com                               1.0.1            Reconcile succeeded
  tmc-local-monitoring          monitoring.tmc.tanzu.vmware.com                    0.0.14           Reconcile succeeded
  tmc-local-stack               tmc-local-stack.tmc.tanzu.vmware.com               0.0.21880        Reconcile succeeded
  tmc-local-stack-secrets       tmc-local-stack-secrets.tmc.tanzu.vmware.com       0.0.21880        Reconcile succeeded
  tmc-local-support             tmc-local-support.tmc.tanzu.vmware.com             0.0.21880        Reconcile succeeded
```
...
```
tanzu package installed update tanzu-mission-control -p tmc.tanzu.vmware.com --version "1.0.1" --values-file values.yaml --namespace tmc-local
```
...
-b projects.registry.vmware.com/tkg/packages/standard/repo:v2.2.0_update.2\
 ...
 ```
 2. Add Repo to TMC Localping proj
